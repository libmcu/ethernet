...
TEST(HomePlug, SLAC_MATCH_REQ) {
	uint8_t expected[] = {
		0x11,0x22,0x33,0x44,0x55,0x66,0x00,0xe0,
		0x4c,0x36,0x07,0x91,0x88,0xe1,0x01,0x7d,
	...
	uint8_t buf[128] = { 0, };
	struct hp_mme_slac_match_cnf cnf = {
		.app_type = 0,
		.sec_type = 0,
		.len = sizeof(struct hp_mme_slac_match_cnf_body),
		.mfield = {
	...
	size_t len = hp_pack_confirm(HP_MMTYPE_SLAC_MATCH,
			(const struct hp_mme_cnf *)&cnf,
			(struct eth *)buf, sizeof(buf));
	LONGS_EQUAL(90, len);
